...
void step(model_t *m, bool rst, bool data)
{
    ...
    // Flag enabled after cycle spent on s4 completes
    // so : 1 0 1 1 X , we only see the flag go up on x inputted

    // This is the sort of thing that AI would normally get wrong that formal ver could catch for us
    bool next_flag = (m->state == S4);
